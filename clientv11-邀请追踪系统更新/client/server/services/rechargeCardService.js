...
class RechargeCardService {
   ...
   * 标记卡密为已使用
   */
  async markCardAsUsed(cardId, userId) {
    const card = await RechargeCard.findById(cardId);
  ...
  /**
   * 使用卡密（保留旧方法以兼容）
   * @deprecated 请使用 validateCard 和 markCardAsUsed 的组合
   */
  async useCard(code, userId) {
    const card = await this.validateCard(code);
    await this.markCardAsUsed(card._id, userId);
    return card;
  }
