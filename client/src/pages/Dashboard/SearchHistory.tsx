                ...
                {filteredHistory.length > 0 ? (
                  filteredHistory.map((record: any, index) => (
                    <tr key={record._id || index} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center space-x-2">
                          {(record.resultCount || record.resultsCount || 0) > 0 ? getStatusIcon('success') : getStatusIcon('failed')}
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            (record.resultCount || record.resultsCount || 0) > 0 ? getStatusColor('success') : getStatusColor('failed')
                          }`}>
                            {(record.resultCount || record.resultsCount || 0) > 0 ? `找到${record.resultCount || record.resultsCount}条` : '无结果'}
                          </span>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <span className={`font-medium ${
                          (record.pointsCost || record.pointsCharged || 0) > 0 ? 'text-red-600' : 'text-green-600'
                        }`}>
                          {(record.pointsCost || record.pointsCharged || 0) > 0 ? `-${record.pointsCost || record.pointsCharged}` : '免费'}
                        </span>
                      </td>
                    </tr>
                  ))
