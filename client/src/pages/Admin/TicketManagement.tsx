...
export const TicketManagement: React.FC = () => {
  ...
  const loadTickets = async () => {
    try {
      ...
      const response = await fetch(`/api/tickets/admin/all?${params.toString()}`, {
        headers: {
      ...
      if (data.success) {
        ...
        const transformedTickets = (data.data.tickets || []).map((ticket: any) => ({
          ...
          username: ticket.userId?.username || 'Unknown',
    ...
    try {
      ...
      if (data.success) {
        ...
        const updatedTicket = {
          ...
          username: data.data.userId?.username || selectedTicket.username,
