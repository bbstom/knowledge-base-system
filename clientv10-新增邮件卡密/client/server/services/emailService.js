  ...
  try {
    ...
    if (template) {
      ...
      const rendered = template.render({
        ...
        email,
        expireMinutes: '10',
        siteName: process.env.SITE_NAME || '信息查询系统',
        siteUrl: process.env.FRONTEND_URL || 'http://localhost:5173',
        logoUrl: `${process.env.FRONTEND_URL || 'http://localhost:5173'}/logo.png`,
        year: new Date().getFullYear()
      });

      mailOptions = {
        from: `"${process.env.SITE_NAME || '信息查询系统'}" <${process.env.SMTP_USER}>`,
        to: email,
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              ...
              <p>&copy; ${new Date().getFullYear()} ${process.env.SITE_NAME || '信息查询系统'}. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
