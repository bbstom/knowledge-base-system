...
export const userApi = {
  ...
  exchangePoints: (amount: number) => api.post('/user/exchange-points', { amount }),
  getCommissionLogs: (page = 1, limit = 20) => api.get('/user/balance-logs', { params: { page, limit } }),
};
...
export const withdrawApi = {
  ...
  withdrawCommission: (amount: number, type: 'usdt' | 'balance', walletAddress?: string) => 
    api.post('/withdraw/commission', { amount, type, walletAddress }),
  getWithdrawHistory: (page = 1, limit = 10, type?: string) => 
    api.get('/withdraw/history', { params: { page, limit, type } }),
};

export const shopApi = {
  getExchangeRate: () => api.get('/shop/exchange-rate'),
};
