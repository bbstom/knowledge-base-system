...
const ExchangePoints: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [rateRes, profileRes, logsRes] = await Promise.all([
        api.get('/shop/exchange-rate'),
        api.get('/user/profile'),
        api.get('/user/balance-logs?limit=10')
      ]);

      if (rateRes.data.success) {
        setExchangeRate(rateRes.data.data);
      }
      ...
      if (logsRes.data.success) {
        const exchangeOnlyLogs = logsRes.data.data.logs.filter(
  ...
  const handleExchange = async () => {
    ...
    try {
      const response = await api.post('/user/exchange-points', {
        amount: pointsAmount
      });
